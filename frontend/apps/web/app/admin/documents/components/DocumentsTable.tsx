          ...
          {rows.map((row) => (
            <tr key={row.ingest_id} data-testid={`row-${row.ingest_id}`}>
              <td className="px-3 py-2">
                ...
                <div className="font-mono text-xs text-neutral-500">{row.filename}</div>
              </td>
              <td className="px-3 py-2 text-neutral-700">{row.revision || '—'}</td>
              <td className="px-3 py-2 text-neutral-700">{row.document_type}</td>
              <td className="px-3 py-2 font-mono text-xs text-neutral-700">{row.asset ?? '—'}</td>
              <td className="px-3 py-2">
                <div className="flex flex-col gap-1">
                  <StatusBadge status={row.status} />
                  {row.failure_reason ? (
                    <span className="text-xs text-danger-fg">{row.failure_reason}</span>
                  ) : null}
                </div>
              </td>
              <td className="px-3 py-2 tabular-nums text-neutral-700">{row.chunk_count}</td>
