    ...
    <section className="space-y-3 print:break-before-page" aria-label="GHGEMP report">
      <header className="flex items-start justify-between gap-3">
        <div className="space-y-1">
          <h2 className="text-lg font-semibold text-neutral-800">{report.report_type}</h2>
      ...
      <Card>
        <dl className="grid grid-cols-2 gap-3 text-sm md:grid-cols-3">
          ...
          <Field label="Tier readiness">
            ...
            <span className="text-xs text-neutral-500">→ {report.tier_status.target_tier}</span>
          </Field>
        ...
        <Card title="Gaps to target tier">
          <ul className="space-y-1 text-sm">
            {report.tier_status.gaps_to_target.map((gap, i) => (
              <li key={i} className="rounded-md border border-warn-border bg-warn-bg/40 p-2">
